...
SRC_INPUT_FILES = $(SRC_DIR)/generic_list.c $(SRC_DIR)/label_list.c $(SRC_DIR)/element_list.c $(SRC_DIR)/parameter_list.c $(SRC_DIR)/string_operations.c $(SRC_DIR)/spice_format.c $(SRC_DIR)/matrix_operations.c $(SRC_DIR)/mna.c $(SRC_DIR)/build_matrices.c $(SRC_DIR)/lu_decomposition.c $(SRC_DIR)/lu_solve.c $(SRC_DIR)/iterative_refinement.c $(SRC_DIR)/main.c
...
all:
	...
	$(CC) $(CC_OPTIONS) $(FLEX_OUTPUT_SOURCE_FILE) $(BISON_OUTPUT_SOURCE_FILE) $(SRC_INPUT_FILES) $(INCLUDE_DIRS) -o $(OUTPUT_FILE)$(OUTPUT_EXTENSION) -lm
